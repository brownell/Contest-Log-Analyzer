# contest_tools/adif_exporters/laqp_adif.py
#
# Purpose: Provides a contest-specific ADIF exporter for the laqp contests.
...
def export_log(log: ContestLog, output_filepath: str):
    """
    Generates a contest-specific ADIF file for a LAQP log.
    """
    ...
    for _, row in df_to_export.iterrows():
        ...
        # LAQP does not have RST in the exchange
        ...
        # --- LAQP Contest-Specific <STATE> Tag Logic ---
        state_value = None
        if pd.notna(row.get('Mult_STPROV')):
            ...
            logging.warning(f"Could not determine LAQP multiplier for NA station: {row.get('Call')}. Setting STATE tag to 'Unknown'.")
            state_value = "Unknown"
        else:
            state_value = "DX"
    ...
    try:
        ...
        logging.info(f"Custom LAQP ADIF log saved to: {output_filepath}")
    except Exception as e:
        logging.error(f"Error exporting custom LAQP ADIF log to '{output_filepath}': {e}")
        raise
